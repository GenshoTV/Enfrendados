...
void jugar(string jugador1, string jugador2,int &puntosMax,string &jugadorMax)
{
    ...
    for (int ronda = 1; ronda <= RONDAS; ronda++)
    {
        ...
        {
            pasarDados(CantidadUsadosJ1,jugador2,dadosStockJ2,jugador1,dadosStockJ1);

        }
        //verifica si se queda sin dados.
        if(verificarGanadorAnticipado(dadosStockJ1))break;
        ...
        {
            pasarDados(CantidadUsadosJ2,jugador1,dadosStockJ1,jugador2,dadosStockJ2);
        }
        //verifica si se queda sin dados.
        if(verificarGanadorAnticipado(dadosStockJ2))break;
...
void pasarDados(int CantidadUsados, string jugadorRecibe, int &stockRecibe, string jugadorEntrega, int &stockEntrega)
{
    if (CantidadUsados > 0)
    {
        stockEntrega -= CantidadUsados;
        stockRecibe += CantidadUsados;
        cout << jugadorEntrega << " pasa " << CantidadUsados << " dado(s) a " << jugadorRecibe << ".\n";
    }
}
...
void penalizar(int &stockJugador, int &stockContrincante)
{
    ...
    if (stockContrincante > 1)
    {
        cout << "Pierdes tu turno y te han penalizado con un dado del contrincante.\n";
        stockContrincante--;
        stockJugador++;
    }
    else
    {
        cout << "Tienes suerte! tu contrincante no puede pasarte dados porque no tiene más de uno." << endl;
    }

}


int  ObtenerDadoObjetivo(string nombre)
{
    // Tirar dados
...
void turnoJugador(string nombre, int &stock, int &puntos, int &dadosUsados, int &stockDadosContrincante)
{
    ...
    int numeroObjetivo =  ObtenerDadoObjetivo(nombre);
    ...
    while (sumaSeleccionada < numeroObjetivo)
    {
        ...
        if (dadosElegidos == 0)
        {
            penalizar(stock,stockDadosContrincante);

            break;
        }
        else if (dadosElegidos < 0 || dadosElegidos > stock)
        {
            cout << "Elegiste un valor fuera de tu stock, por lo tanto se te penalizara!"<< endl;
            penalizar(stock,stockDadosContrincante);
            break;
        }
        else if (usados[dadosElegidos - 1])
        {
            cout << "Ese dado ya fue utilizado.Por favor elege otro dado." << endl;
            continue;
        }
        ...
        {
            cout << "Felicitaciones! Utilizaste todos tus dados, ganaste automaticamente y sumaste 10.000 puntos!\n" << endl;
            puntos += 10000;
        ...
        {
            cout << "Te pasaste del objetivo!, fuiste penalizado" << endl;
            penalizar(stock,stockDadosContrincante);
        }
